...
describe('Accordion.cy.jsx', () => {
  it('Accordion function tests', () => {
    cy.mount(<ItemsAccordion items={items}/>);
    cy.getDataTest('accordion-wrapper').within(()=>{
      cy.get('[data-test^="accordions-item"]').should('have.length',8);
      cy.getDataTest('accordions-item-1').within(()=>{
        cy.contains(/Your tests will exist in a describe block./i).should('not.be.visible');
      })
      cy.getDataTest('accordions-item-1').find('[role=button]').click();
      cy.getDataTest('accordions-item-1').within(()=>{
